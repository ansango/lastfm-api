...
const RESERVED_FIELDS = ['method', 'api_key', 'api_sig', 'format'];
/**
 ...
 */
export async function signedPost(config, method, options) {
    const requiresSession = options.requiresSession ?? true;
    // 1. Shared secret must exist before we touch anything.
    if (!config.sharedSecret) {
        throw new LastFmApiError('A `sharedSecret` is required for signed methods. Pass `sharedSecret` in the LastFmConfig.', 0);
    }
    ...
    if (requiresSession) {
        ...
        if (!resolvedSk) {
            throw new LastFmApiError('A session key (`sk`) is required for this method. Pass `sk` in the request or set `sessionKey` on the LastFmConfig.', 0);
        }
