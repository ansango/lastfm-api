...
import type { InsightsNowPlayingRequest, InsightsNowPlayingResponse } from '../insights.schemas.js'
import { createUserService } from '../user.js'

/**
 * Strips Last.fm wiki markup and HTML tags from biography text.
 */
...
/**
 * Truncates biography text cleanly on a word boundary.
 */
...
/**
 * Finds the highest quality image available from a Last.fm image array.
 */
...
export async function getNowPlaying(
	...
	if (artistName.length > 0) {
		try {
			...
			// Best-effort enrichment: if artist lookup fails, still return track & artist data
		}
	}
