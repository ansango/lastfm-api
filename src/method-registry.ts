...
const NS_DEFAULTS: Readonly<Record<string, Pick<MethodMeta, 'httpMethod' | 'bodyKind' | 'requiresSession' | 'requiresSignature'>>> = {
...
function buildMethodMeta(id: string): MethodMeta {
	...
	if (!response) {
		...
		response = (cfg.schemas[tagMutKey] as ZodTypeAny | undefined) ??
			(cfg.schemas[mutKey] as ZodTypeAny | undefined) ??
			undefined;
	}
	if (!response) {
		throw new Error(
...
for (const id of CANONICAL_METHODS) {
	...
	(registry[meta.ns] ??= {})[meta.name] = meta;
}
