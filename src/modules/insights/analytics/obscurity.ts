...
import type { InsightObscureArtist, InsightsObscurityRequest, InsightsObscurityResponse } from '../schemas.js'
...
export async function getObscurityScore(
	...
	const infoPromises = artistList.map(async (art) => {
		const name = art.name
		const userPlaycount = Number.parseInt(String(art.playcount ?? '0'), 10) || 0
		try {
			const infoRes = await artistService.getInfo({ artist: name }, init)
			const globalListeners = Number.parseInt(String(infoRes.artist?.stats?.listeners ?? '0'), 10) || 0
			const globalPlaycount = Number.parseInt(String(infoRes.artist?.stats?.playcount ?? '0'), 10) || 0
