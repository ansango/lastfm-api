...
/**
 * Compares 3-10 users simultaneously, computing pairwise Jaccard affinities,
 * group consensus artists, and identifying group outliers and taste anchors.
 */
export async function compareTasteGroup(
	...
	// Fetch top artists for all users in parallel
					...
					for (const a of list) {
						if (a.name) {
							artistMap.set(a.name.toLowerCase().trim(), {
								name: a.name,
								playcount: Number.parseInt(a.playcount ?? '0', 10) || 1,
							})
						}
					}
	...
	// Overall group average compatibility
	...
	// Determine Outlier (lowest avg compatibility) and Anchor (highest avg compatibility)
	...
	// Consensus artists: aggregated across all users
	...
	// Filter artists heard by at least 2 users (or >= 50% of the group)
