...
import { parse, buildMarkdown, type FileSection } from '../../tool/ci-summary.js';
...
describe('parse()', () => {
	test('parses bun output without group prefix (local default)', () => {
		...
		expect(sections[0].tests[0]).toEqual({ status: 'pass', name: 'album service > getInfo > routes to album.getInfo', ms: 1.5 });
	...
	test('accepts ##[group] prefix (how the runner renders the same bytes)', () => {
		const input = [
			'##[group]src/__tests__/track.test.ts:',
			'(pass) track > getInfo [0.5ms]',
			'##[endgroup]'
		].join('\n');
...
describe('buildMarkdown()', () => {
	...
	test('picks ✅ when everything passed and ⚠️ when only skipped remain', () => {
		...
		expect(buildMarkdown([{ name: sections[0].name, tests: onlySkip }], '1', 'x')).toContain(
			'## ⚠️'
		);
	});
