...
import { fakeTag, fakeTrack, LAST_FM_ERROR_CODES, lastFmError } from './fixtures/lastfm-responses.js'
...
describe('track service', () => {
	...
	describe('error handling', () => {
		test('Last.fm error envelope becomes LastFmApiError', async () => {
			...
			await expect(client.track.getInfo({ artist: 'Test Artist', track: 'Nonexistent' })).rejects.toBeInstanceOf(
				LastFmApiError,
			)
		})
	})
	...
	describe('updateNowPlaying (regression — restored after #89 merge drop)', () => {
		...
		test('Last.fm error envelope becomes LastFmApiError', async () => {
			...
			await expect(authed.track.updateNowPlaying({ artist: 'A', track: 'T' })).rejects.toBeInstanceOf(LastFmApiError)
		})
	})
