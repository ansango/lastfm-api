...
const AUTH_TAG_DESCRIPTION = [
	'## Auth flow',
	'',
	'Last.fm has three auth flows (web, desktop, mobile). For self-service API keys — the only kind the public create form issues — **the web flow is the one that works.** The mobile flow is restricted to mobile-classified keys, which Last.fm does not expose through the public form, and the desktop flow is operationally identical to the web flow (same two API calls, just with a localhost callback URL). This tool leads with the web flow.',
	'',
	'### 1. Get a session key — web flow (recommended)',
	'',
	'**a.** `GET /auth/get-token` — returns a request token. Signed, no session needed.',
	'',
	'**b.** Open `https://www.last.fm/api/auth/?api_key=<LASTFM_API_KEY>&token=<token>` in a browser. Log in if prompted, click "Allow access". Last.fm redirects to the callback URL configured on your API account with the same `token` in the URL bar.',
	'',
	...
	'',
	'### 3. Mobile flow — deprecated, mobile-class keys only',
	'',
