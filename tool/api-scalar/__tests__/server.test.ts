...
describe('createApp: shape', () => {
	...
	test('GET /doc returns valid OpenAPI 3.0 JSON with 79 paths and x-tagGroups', async () => {
		...
		expect(Object.keys(body.paths).length).toBe(79)
		expect(body['x-tagGroups']).toBeDefined()
		expect(body['x-tagGroups']?.length).toBe(3)
